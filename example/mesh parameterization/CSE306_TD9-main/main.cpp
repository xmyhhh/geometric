...
int tuttle() {
	...
	for (int i = 0; i < mesh.indices.size(); i++) {
		int vtx1 = mesh.indices[i].vtxi;   //对点的index
		...
		vtx_to_tri[vtx1].push_back(i);  //点和面之间的关系，点1=面1，面2……
	...
	for (int i = 1; i < boundary_edges.size(); i++)
	{
		...
		{
			//如果边界j的第一个顶点和ordered_boundary_edges中i-1个边界的第二个顶点index相同,更新ordered_boundary_edges中i个边界
	...
	for (int iter = 0; iter < 5000; iter++)
	{
		...
		for (int i = 0; i < mesh.vertices.size(); i++)
		{
			...
			for (int j = 0; j < vtx_to_tri[i].size(); j++)//第i个顶点的j个相邻面
			{
				if (mesh.indices[vtx_to_tri[i][j]].vtxi != i)  //第i个顶点的j个相邻面的1号顶点
					avg_neighbors += mesh.vertices[mesh.indices[vtx_to_tri[i][j]].vtxi];
				if (mesh.indices[vtx_to_tri[i][j]].vtxj != i)  //第i个顶点的j个相邻面的2号顶点
					avg_neighbors += mesh.vertices[mesh.indices[vtx_to_tri[i][j]].vtxj];
				if (mesh.indices[vtx_to_tri[i][j]].vtxk != i)  //第i个顶点的j个相邻面的3号顶点
...
int abf() {
	...
	auto OpenABFMesh = ABF::Mesh::New();

	for (int i = 0; i < mesh.vertices.size(); i++) {

		OpenABFMesh->insert_vertex(mesh.vertices[i].data[0], mesh.vertices[i].data[1], mesh.vertices[i].data[2]);
	}
	for (int i = 0; i < mesh.indices.size(); i++) {
		int vtx1 = mesh.indices[i].vtxi;
		int vtx2 = mesh.indices[i].vtxj;
		int vtx3 = mesh.indices[i].vtxk;
		OpenABFMesh->insert_face(vtx1, vtx2, vtx3);
	}
	// Print new coordinates
	int i = 0;
	for (const auto& v : OpenABFMesh->vertices()) {
	...
	std::cout << "--------------------------" << std::endl;
	ABF::Compute(OpenABFMesh);
	LSCM::Compute(OpenABFMesh);
	...
	for (const auto& v : OpenABFMesh->vertices()) {
		...
		if (i < 10)
			std::cout << v->idx << ": " << v->pos << std::endl;
		mesh.vertices[i].data[0] = v->pos[0];
		mesh.vertices[i].data[1] = v->pos[1];
		mesh.vertices[i].data[2] = v->pos[2];
		i++;
	}



	mesh.writeOBJ("flattened_abf.obj");
	return 0;
}
