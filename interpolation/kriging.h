...
public:
	...
	Kriging(std::vector<DataPoint>& _dataPoint, RasterContext& rasterContext);

	Kriging(std::vector<DataPoint>& _dataPoint) { dataPoint.swap(_dataPoint); };
	// Destructor
	...
	void SimpleKrige(Model model, double nugget, double sill, double range, concurrency::concurrent_vector <DataPoint>& dataPoint_value);

	// Ordinary Kriging

	void OrdinaryKrige(Model model, double nugget, double sill, double range,
		unsigned int minPoints, unsigned int maxPoints, double maxDistance, concurrency::concurrent_vector<DataPoint>& dataPoint_value);
...
void Kriging::SimpleKrige(Model model, double nugget, double sill, double range, concurrency::concurrent_vector <DataPoint>& dataPoint_value)
{
	...
	unsigned int minPoints, unsigned int maxPoints, double maxDistance, concurrency::concurrent_vector <DataPoint>& dataPoint_value)
{
	...
	bool use_prallel = false;
	if (use_prallel)
		Concurrency::parallel_for(0, (int)rasterContext.width, [&](int i) {
		for (unsigned int j = 0; j < rasterContext.height; j++)
		{
			Point point = rasterContext.XYtoPoint(i, j);
...
bool KrigeZoneAData(std::vector<DataPoint>& dataPoint, concurrency::concurrent_vector <DataPoint>& dataPoint_value)
{
	bool result = false;
	...
	if (result)
	{
		...
		std::vector<DataPoint> dataPoint_copy(dataPoint);

		Kriging kriging(dataPoint_copy, rasterContext);
		...
		kriging.SimpleKrige(Kriging::Spherical, 0.0, kriging.GetEstimatedSill(), 4000, dataPoint_value);

		//kriging.OrdinaryKrige(Kriging::Spherical, 0.0, kriging.GetEstimatedSill(), 2000, 12, 12, 0, dataPoint_value);
